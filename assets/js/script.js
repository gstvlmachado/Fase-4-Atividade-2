...
function requisitoUm() {
    let regex = /\*([^*]+)\*/g;
    let array = texto.match(regex);

    for (i in array) {
        let requisitoUm = [];
        requisitoUm = array[i].replace(/\*/g, '');
        console.log(requisitoUm);
    }
}
requisitoUm();
...
function requisitoDois() {
    let regex = /#Roteiro A \|([^#]+)/g;
    let array = [];

    array = texto.match(regex);

    for (i in array) {
        let requisitoDois = [];
        requisitoDois = array[i].replace(/<[^>]+>/g, ' ').replace(/;/g, ',');
        console.log(requisitoDois);
    }
}
requisitoDois();
